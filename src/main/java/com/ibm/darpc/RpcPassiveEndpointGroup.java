...
public class RpcPassiveEndpointGroup<R extends RdmaRpcMessage, T extends RdmaRpcMessage> extends RpcEndpointGroup<R,T> {
	...
	public static <R extends RdmaRpcMessage, T extends RdmaRpcMessage> RpcPassiveEndpointGroup<R,T> createDefault(RdmaRpcService<R, T> rpcService, long clusterAffinities[], int timeout, int maxinline, boolean polling, int rpcpipeline, int maxSge, int cqSize) throws Exception {
		RpcPassiveEndpointGroup<R,T> group = new RpcPassiveEndpointGroup<R,T>(rpcService, clusterAffinities, timeout, maxinline, polling, rpcpipeline, maxSge, cqSize);
		RpcEndpointFactory<R,T> factory = new RpcEndpointFactory<R,T>(group);
		group.init(factory);
		return group;
	}
	
	protected RpcPassiveEndpointGroup(RdmaRpcService<R, T> rpcService, long clusterAffinities[], int timeout, int maxinline, boolean polling, int rpcpipeline, int maxSge, int cqSize) throws Exception {
		super(rpcService, clusterAffinities, timeout, maxinline, polling, rpcpipeline, maxSge, cqSize);
		logger.info("rpc passive endpoint group (2), maxWR " + rpcpipeline + ", maxSge " + maxSge + ", cqSize " + cqSize);
	}	
	...
	public RdmaCqProvider createCqProvider(RpcEndpoint<R,T> endpoint)
		...
		if (endpoint.isServerSide()){
			logger.info("setting active cq processor");
	...
	public static class RpcEndpointFactory<R extends RdmaRpcMessage, T extends RdmaRpcMessage> implements RdmaEndpointFactory<RpcEndpoint<R,T>> {
		...
		public RpcEndpoint<R,T> createEndpoint(RdmaCmId id, boolean serverSide) throws IOException {
			return new RpcPassiveEndpoint<R,T>(group, id, serverSide);
		}
	}	
}
